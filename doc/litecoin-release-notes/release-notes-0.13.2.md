Aocoin Core version 0.13.2 is now available from:

  <https://download.aocoin.org/aocoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/aocoin-project/aocoin/issues>
...
updates will be released anymore. Without security updates, using a aocoin
...
ECDSA signatures inside Aocoin transactions now use validation using
...
Previous versions of Aocoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Aocoin Core was to raise the default minimum
relay fee.

Aocoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Aocoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running aocoind without having to do any manual configuration.
...
Aocoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Aocoin Core will relay transactions with
...
entirely in Aocoin Core 0.13, and it is at this time undecided whether the
...
Aocoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Aocoin Core automatically creates a hidden service to listen on, without
manual configuration. Aocoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Aocoin Core is listening, and
...
Aocoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Aocoin
Core to use floating fees. By default, Aocoin Core will use floating
fees.
...
(default: `0.0002` AOC/kB).

At all times, Aocoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) AOC.
Furthermore, Aocoin Core will never create transactions paying less than
...
`aocoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinrpc>
        ...
        # optional bypass aocoind rpc basic auth
...
Restarting aocoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in aocoin.conf
...
For example:
    ...
    $ src/aocoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Aocoin Core code base has
...
- `aocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `aocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Aocoin scripting system that in combination with
...
Aocoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Aocoin Core.
...
The two phases are distinct in the Aocoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Aocoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Aocoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Aocoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Aocoin addresses (both P2PKH addresses
  ...
  believed to be provided by Aocoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Aocoin's current ledger (called the UTXO set),
  ...
  note that Aocoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Aocoin users to individually opt-in to almost any change in the Aocoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Aocoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Aocoin
client to Aocoin Core 0.13.2 to signal for segwit support. When a
...
Since Aocoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `aocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
